...
function displayMatches() {
	...
	const html = matchArray.map(restaurants => {
		...
		return `
			...
			<address class = "address">${restaurants.address_line_1}</address>
			<address class = "city">${restaurants.city}</address>
			<address class = "category">${restaurants.category}</address>
			<address class = "zip">${restaurants.zip}</address>
		`;
	}).join('');
